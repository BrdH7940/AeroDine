      ...
      return (
        <button
          onClick={() => {
            ...
            alert(`Order for Table ${ticket.tableNumber} completed!`);
          }}
          className="w-full px-4 py-2 bg-slate-500 hover:bg-slate-600 text-white rounded-lg font-medium transition-colors flex items-center justify-center gap-2"
        >
          <CheckCircle size={18} />
          Done
        </button>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3 pb-3 border-b border-slate-100">
        <div>
          <h3 className="text-lg font-semibold text-slate-900">Table {ticket.tableNumber}</h3>
          <p className="text-xs text-slate-500">Ordered at {ticket.orderTime}</p>
        </div>
        <div
            ...
            {item.modifiers && item.modifiers.length > 0 && (
              <div className="mt-1 ml-4 space-y-0.5">
                {item.modifiers.map((modifier, modIndex) => (
                  <p key={modIndex} className="text-xs text-slate-500">
                    - {modifier}
                  </p>
                ))}
              </div>
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">Manage and track order preparation</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {columns.map((column) => (
          <div key={column.status} className="flex flex-col">
            {/* Column Header */}
            <div
              className={`bg-white rounded-t-lg border-t-4 ${column.borderColor} p-4 shadow-sm`}
            >
            ...
            {/* Tickets */}
            <div className="flex-1 bg-slate-50 rounded-b-lg p-4 space-y-4 min-h-[600px]">
